...
export default function EmptyState({ icon: Icon, message, actionLabel, onAction }) {
  return (
    <div style={{ padding: '2.75rem 1.5rem', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
      {Icon && (
        <div style={{ width: 48, height: 48, borderRadius: '50%', background: C.expandBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.textFaint }}>
          <Icon size={22} />
        </div>
      )}
      ...
      {actionLabel && onAction && (
        <button
          onClick={onAction}
          style={{ marginTop: 2, padding: '9px 18px', background: 'none', border: `1px solid ${C.border}`, borderRadius: 8, cursor: 'pointer', color: C.greenLight, fontFamily: font, fontSize: 13.5, fontWeight: 500 }}
        >
          {actionLabel}
        </button>
      )}
